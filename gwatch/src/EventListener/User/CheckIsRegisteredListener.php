...
class CheckIsRegisteredListener
{
    /** @var RouterInterface */
    protected $router;
    ...
    public function onKernelController(FilterControllerEvent $event)
    {
        ...
        if (true === $this->session->get('user_is_registered')) {
            return;
        }
        ...
        if ($user = $this->entityManager
            ...
            $this->session->set('user_is_registered', true);
        }

        if (!$user && $this->session->get('pending_user_is_registered')) {
            $this->session->getFlashBag()->add(
                'warning',
                'Your user access is being set up, this may take up to 30 seconds. Please try again shortly.'
            );
            ...
            $this->session->getFlashBag()->add(
                'warning',
                'You cannot access the user section until you become a registered user. Please complete the form below to proceed.'
            );
        }
